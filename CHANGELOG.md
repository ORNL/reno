...
* Component `cgroup` and model group handling variables in output JSON

### Fixed

* Parser incorrectly handling references within distributions
* String outputs of scalars of numpy arrays not including commas, breaking parsing
...
* `sampling_kwargs` not passing through to `sample_prior_predictive` calls
...
* A configuration dataframe pane for explorer
...
* `mean` series operation

### Fixed

* Diagrams breaking on certain TrackedReferences not including cgroup/group
* `Index` op not evaluating index equation
