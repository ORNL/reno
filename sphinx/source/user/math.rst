...
``EquationPart``) which has two child nodes, each a constant scalar value.

No math has actually occurred yet, to execute you can run ``eq.eval()``, which
recursively evaluates and returns the equation value. (A
...
link to components) for example:
...
running the :py:class:`reno.ops.orient_timeseries` on the component, or more
semantically friendly by calling the component's :py:attr:`timeseries
